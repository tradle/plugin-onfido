...
class Onfido {
    constructor(opts) {
        this['onmessage:tradle.Form'] = (req) => __awaiter(this, void 0, void 0, function* () {
            ...
            let fresh;
            try {
                const { permalink } = yield this.bot.kv.get(getStateKey(application));
            ...
            if (fresh) {
                yield this.productsAPI.sign(state);
                ...
                yield this.productsAPI.version(state);
                yield this.productsAPI.save(state);
            }
        });
        ...
        this.processWebhookEvent = ({ req, res, desiredResult }) => __awaiter(this, void 0, void 0, function* () {
            ...
            let webhook;
            try {
                webhook = yield this.bot.conf.get(constants_2.ONFIDO_WEBHOOK_KEY);
            }
            catch (err) {
        ...
        this.handleForm = ({ req, application, state, form }) => __awaiter(this, void 0, void 0, function* () {
            const { type } = req;
            const { result, pendingCheck, onfidoApplicant, selfie, photoID } = state;
            if (!onfidoApplicant) {
                try {
                    yield this.applicants.createOrUpdate({ req, application, state, form });
                }
                catch (error) {
            ...
            if (!pendingCheck) {
                yield this.updateApplicantAndCreateCheck({ req, application, state, form });
                return;
            }
            if (!result) {
        ...
        this.updateApplicantAndCreateCheck = ({ req, application, state, form }) => __awaiter(this, void 0, void 0, function* () {
            try {
            ...
            if (!state.selfie) {
                const selfie = type === constants_2.SELFIE ? form : utils_1.getSelfie(application);
                if (selfie) {
            ...
            if (!state.photoID) {
                const photoID = type === constants_2.PHOTO_ID ? form : utils_1.getPhotoID(application);
                if (photoID) {
            ...
            try {
                yield this.createCheck({ application, state });
            }
            finally {
                yield this.productsAPI.save(state);
            }
        });
        this.createCheck = ({ application, state }) => __awaiter(this, void 0, void 0, function* () {
            return yield this.checks.create({
                application,
                state,
                reports: onfido_models_1.default.reportType.enum.map(({ id }) => id)
            });
        });
        const { logger, onfidoAPI, products, productsAPI, padApplicantName, formsToRequestCorrectionsFor, } = opts;
        this.logger = logger;
...
exports.default = opts => new Onfido(opts);
