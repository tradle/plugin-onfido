...
const { getSelfie, getPhotoID, firstProp, parseReportURL, parseCheckURL, getOnfidoCheckIdKey, parseStub, getLatestFormByType, addLinks, validateProductOptions, getFormStubs, getEnumValueId, } = utils;
...
class Onfido {
    constructor(opts) {
        ...
        this.getPropertyMap = (productModelId) => {
        ...
        this.getForm = async ({ type, application, form, req }) => {
            ...
            else if (req && req.payload && req.payload[constants_1.TYPE] === type) {
        ...
        this.shouldIgnoreForm = ({ product, form }) => {
            ...
            if (!utils.isAddressRequired(productOpts.reports)) {
                const relevant = Object.keys(constants_2.PROPERTY_SETS).filter(name => {
                    return constants_2.PROPERTY_SETS[name].some(fieldName => utils.canExtractFromFormType({
                        formType: form,
                        fieldName,
                        propertyMap
                    }));
        ...
        this.products = products.map(opts => {
            return Object.assign({}, opts, { reports: opts.reports || constants_2.DEFAULT_REPORTS, propertyMap: opts.propertyMap || DEFAULT_PROPERTY_MAP });
        });
        this.bot = bot;
...
const getReportsOrderedEnumVals = (reports) => reports.map(id => buildResource.enumValue({
    model: onfido_models_1.default.reportType,
    value: id
}));
const getAspects = (reports) => reports.map(report => constants_2.REPORT_TO_ASPECT[report]);
