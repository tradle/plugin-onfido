...
function mockClient(opts = {}) {
    ...
    const bot = mockBot();
    const plugin = _1.createPlugin(Object.assign({ logger: new console_logger_1.default(), bot,
        onfidoAPI, productsAPI: {
            ...
            state: {
                getLatestFormByType: (forms, type) => {
                    return forms.slice().reverse().find(({ id }) => utils_1.parseStub(id).type === type);
                }
            }
        } }, opts));
    plugin.conf.put(constants_2.DEFAULT_WEBHOOK_KEY, { token: 'testtoken' });
    return plugin;
}
function mockBot() {
    const db = {};
    ...
    const signAndSave = (resource) => __awaiter(this, void 0, void 0, function* () {
        const signed = yield sign(resource);
