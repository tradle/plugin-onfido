...
const setup = () => {
    ...
    const application = {
        [constants_1.TYPE]: constants_2.APPLICATION,
...
test('common case', loudAsync((t) => __awaiter(this, void 0, void 0, function* () {
    let { onfido, state, application, completedCheck, pendingCheck, user } = setup();
    onfido.bot.users.get = (id) => __awaiter(this, void 0, void 0, function* () {
        if (id !== user.id) {
    ...
    const importVerificationStub = sinon.stub(onfido.applications, 'createVerification')
        .callsFake(({ verification }) => __awaiter(this, void 0, void 0, function* () {
    ...
    sinon.stub(onfido.applications, 'requestEdit').callsFake(({ item, details }) => __awaiter(this, void 0, void 0, function* () {
        t.equal(details.errors[0].name, 'postcode');
    }));
    yield onfido.handleOnfidoError({
    ...
    t.ok(state.photoID);
    yield onfido.createCheck({ application, state, saveState: true });
    t.equal(state.result, undefined);
    t.ok(state.check);
    t.same(state.checkStatus, { id: 'tradle.onfido.CheckStatus_inprogress', title: 'In progress' });
    let reportIdx = 0;
    sinon.stub(onfido.onfidoAPI.webhooks, 'handleEvent').callsFake((req, token) => __awaiter(this, void 0, void 0, function* () {
    ...
    sinon.stub(onfido.onfidoAPI.checks, 'get').callsFake((props) => __awaiter(this, void 0, void 0, function* () {
    ...
    for (let i = 0; i < pendingCheck.reports.length; i++) {
        yield onfido.processWebhookEvent({ req: mock_1.default.request() });
    }
    yield onfido.processWebhookEvent({ req: mock_1.default.request() });
    t.equal(vIdx, 3);
...
test.only('plugin methods', loudAsync((t) => __awaiter(this, void 0, void 0, function* () {
    const { onfido, 
    // state,
    application, completedCheck, pendingCheck } = setup();
    ...
    yield Promise.all(Object.keys(forms).map(name => {
        return onfido.bot.save(forms[name]);
    }));
    const receive = (payload) => __awaiter(this, void 0, void 0, function* () {
        yield onfido['onmessage:tradle.Form']({ payload, application });
        application.forms.push(toAppSub(payload));
        yield onfido.bot.save(application);
    });
    yield receive(forms.name);
    ...
    yield receive(forms.driving_license);
    t.equal(createApplicantStub.callCount, 0);
    t.equal(uploadLivePhotoStub.callCount, 0);
    t.equal(uploadDocumentStub.callCount, 0);
    t.equal(updateApplicantSpy.callCount, 0);
    t.equal(createCheckStub.callCount, 0);
    yield receive(forms.applicant);
    t.equal(createApplicantStub.callCount, 1);
    t.equal(uploadLivePhotoStub.callCount, 0);
    t.equal(uploadDocumentStub.callCount, 1);
    t.equal(updateApplicantSpy.callCount, 0);
    t.equal(createCheckStub.callCount, 0);
    yield receive(forms.selfie);
    t.equal(createApplicantStub.callCount, 1);
