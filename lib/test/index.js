...
const setup = () => {
    ...
    const completedCheck = clone(fixtures_1.default.checks.complete);
    const pendingCheck = clone(fixtures_1.default.checks.pending);
    const state = {
...
test('common case', loudAsync((t) => __awaiter(this, void 0, void 0, function* () {
    let { onfido, state, application, completedCheck, pendingCheck } = setup();
    ...
    const importVerificationStub = sinon.stub(onfido.productsAPI, 'importVerification')
        ...
        verification = omit(verification, constants_1.SIG);
        if (vIdx === 0) {
